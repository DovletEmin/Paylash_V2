...
const SharesPage = {
        ...
        if (isAdmin) {
            ...
            <div class="form-group">
                ...
                <div class="visibility-buttons" id="vis-buttons">
                    ...
                    <button class="btn btn-sm vis-btn ${vis === 'common' ? 'active' : ''}" data-vis="common" onclick="SharesPage.selectVisibility('common')">🌐 Umumy</button>
                </div>
                <div id="vis-status" class="vis-status" style="font-size:.78rem;color:var(--text-3);margin-top:6px">
                    ${vis === 'common' ? 'Ähli işgärler görüp we redaktirläp biler' : 'Diňe siz we paýlaşylanlar'}
                </div>
            </div>`;
        }
    ...
    selectVisibility(vis) {
        ...
        if (st) st.textContent = vis === 'common' ? 'Ähli işgärler görüp we redaktirläp biler' : 'Diňe siz we paýlaşylanlar';
    },
