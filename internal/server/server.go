...
func (s *Server) routes(webFS embed.FS) {
	...
	s.mux.Handle("PATCH /api/chat/conversations/{id}", auth(http.HandlerFunc(h.RenameConversation)))
	s.mux.Handle("POST /api/chat/conversations/{id}/participants", auth(http.HandlerFunc(h.AddParticipants)))
	s.mux.Handle("DELETE /api/chat/conversations/{id}/participants/{userId}", auth(http.HandlerFunc(h.RemoveParticipant)))
	s.mux.Handle("GET /api/chat/conversations/{id}/messages", auth(http.HandlerFunc(h.ListMessages)))
	s.mux.Handle("POST /api/chat/conversations/{id}/messages", auth(http.HandlerFunc(h.SendMessage)))
	...
	s.mux.Handle("GET /api/chat/unread-count", auth(http.HandlerFunc(h.ChatUnreadCount)))
	s.mux.Handle("PATCH /api/chat/notification-prefs", auth(http.HandlerFunc(h.UpdateNotificationPrefs)))
	s.mux.Handle("GET /api/chat/ws", auth(http.HandlerFunc(h.ChatWebSocket)))
	s.mux.Handle("PATCH /api/chat/conversations/{id}/participants/{userId}/role", auth(http.HandlerFunc(h.SetParticipantRole)))
	s.mux.Handle("PATCH /api/chat/conversations/{id}/prefs", auth(http.HandlerFunc(h.UpdateConversationPrefs)))
	s.mux.Handle("GET /api/chat/muted-conversations", auth(http.HandlerFunc(h.ListMutedConversations)))
	s.mux.Handle("POST /api/chat/conversations/{id}/messages/{messageId}/pin", auth(http.HandlerFunc(h.PinMessage)))
	s.mux.Handle("DELETE /api/chat/conversations/{id}/pinned-message", auth(http.HandlerFunc(h.UnpinMessage)))
	s.mux.Handle("POST /api/chat/conversations/{id}/avatar", auth(http.HandlerFunc(h.UploadGroupAvatar)))
	s.mux.Handle("GET /api/chat/conversations/{id}/avatar", auth(http.HandlerFunc(h.ServeGroupAvatar)))
